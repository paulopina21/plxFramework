                                               ...
                                               const QString &fname, QString currentPath, bool ignoreErrors)
{
      ...
      case QXmlStreamReader::Characters:
        ...
        else
        {
          ...
          if (fileName.isEmpty())
          {
            ...
            CLog::log(LOG_WARNING, "RCC_File", msg.toUtf8());
          }
  ...
  if (reader.hasError())
  {
    ...
    QString msg = QString::fromLatin1("Parse Error: '%1' Line: %2 Column: %3 [%4]\n").arg(fname).arg(errorLine).arg(errorColumn).arg(errorMessage);
    CLog::log(LOG_ERROR, "RCC_File", msg.toUtf8());
    return false;
  }

  if (m_root == 0)
  {
    const QString msg = QString::fromUtf8("No resources in '%1'.\n").arg(fname);
    CLog::log(LOG_WARNING, "RCC_File", msg.toUtf8());
...
bool RCCResourceLibrary::addFile(const QString &alias, const RCCFileInfo &file)
{
  ...
  if (parent->m_children.contains(filename))
  {
    foreach (const QString &fileName, m_fileNames)
      CLog::log(LOG_WARNING,"RCC_FILE", "Potential duplicate alias detected: " + fileName + ", " + filename);
  }
...
bool RCCResourceLibrary::readFiles(bool ignoreErrors, QIODevice &errorDevice)
{
  ...
  if (m_verbose) {
    ...
    CLog::log(LOG_INFO, "RCC_File", msg.toUtf8());
  }
  for (int i = 0; i < m_fileNames.size(); ++i) {
    ...
    if (fname == QLatin1String("-")) {
      ...
      if (!fileIn.open(stdin, QIODevice::ReadOnly)) {
        CLog::log(LOG_ERROR, "RCC_File", msgOpenReadFailed(fname, fileIn.errorString()).toUtf8());
        return false;
      }
    } else {
...
bool RCCResourceLibrary::addFile(const QString& strFilePath, const QString& strAlias, const QString& strPrefix)
{
  QString alias = strAlias;
  QString prefix = strPrefix;
  ...
  QString absFileName = strFilePath;
  ...
  if (!file.exists())
  {
    CLog::log(LOG_ERROR, "RCC_FILE", "File not found");
    m_failedResources.push_back(file.absoluteFilePath());
    return false;
  }
  ...
  CLog::log(LOG_DEBUG, "RCC_FILE", "Prefix: " + prefix);
  CLog::log(LOG_DEBUG, "RCC_FILE", "Alias: " + alias);


  addFile(strAlias,
          RCCFileInfo(strAlias.section('/', -1),
                      file)
          );
}
...
bool RCCResourceLibrary::output(QIODevice &outDevice, QIODevice &errorDevice)
{
  //write out
  if (m_verbose)
    CLog::log(LOG_INFO, "RCC_File", "Outputting code\n");

  if (!writeHeader()) {
    CLog::log(LOG_ERROR, "RCC_File", "Could not write header\n");
    return false;
  }
  if (m_root) {
    if (!writeDataBlobs()) {
      CLog::log(LOG_ERROR, "RCC_File", "Could not write data blobs.\n");
      return false;
    }
    if (!writeDataNames()) {
      CLog::log(LOG_ERROR, "RCC_File", "Could not write file names\n");
      return false;
    }
    if (!writeDataStructure()) {
      CLog::log(LOG_ERROR, "RCC_File", "Could not write data tree\n");
      return false;
    }
  }
  if (!writeInitializer()) {
    CLog::log(LOG_ERROR, "RCC_File", "Could not write footer\n");
    return false;
  }
...
bool RCCResourceLibrary::save(QIODevice& outDevice)
{
  //write out
  if (m_verbose)
    CLog::log(LOG_INFO, "RCC_File", "Outputting code\n");
  ...
  while (!pending.isEmpty()) {
    ...
    for (QHash<QString, RCCFileInfo*>::iterator it = file->m_children.begin();
      ...
      if (child->m_flags & RCCFileInfo::Directory)
        ...
        if (offset == 0) {
          CLog::log(LOG_ERROR, "RCC_File", errorMessage.toUtf8());
          return false;
        }
      }
    }
